...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (stats && stats.remainingBalance < 10000 && !warningShown) {
      console.log("Triggering low balance warning:", {
      ...
      showWarning(
        "Low Balance Alert!",
        `Only ₹${stats.remainingBalance.toLocaleString()} remaining. Email notification sent.`
      );
      sonner.warning(
        "Critical Balance Warning",
        `You have only ₹${stats.remainingBalance.toLocaleString()} left this month - below ₹10,000 threshold!`
      );
      setWarningShown(true);
    } else if (stats && stats.remainingBalance >= 10000 && warningShown) {
      // Reset warning state if balance goes back above threshold
      console.log("Resetting warning state - balance back above threshold");
  ...
  const handleAddExpense = async (e: React.FormEvent) => {
    ...
    try {
      ...
      sonner.success(
        "Expense added!",
        `₹${newExpense.amount} added to ${newExpense.category}`
      );
      showSuccess(
        "Expense Added",
        `Successfully added ₹${newExpense.amount} expense`
      );
  ...
  const handleSeedData = async () => {
    ...
    try {
      ...
      sonner.success(
        "Sample data added!",
        "Your dashboard now has sample expenses"
      );
      showSuccess(
        "Sample Data Added",
        "Sample expenses have been added to your account"
      );
      fetchStats();
    } catch (error: any) {
      sonner.dismiss(loadingId);
      const errorMessage =
        error.response?.data?.message || "Error adding sample data";
      sonner.error("Error", errorMessage);
      showError("Error", errorMessage);
    } finally {
      setIsSeeding(false);
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              ...
              <div
                ...
                } rounded-2xl p-6 text-white shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105 cursor-pointer`}
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-sm">
                      {stats.balancePercentage.toFixed(1)}% left
                    </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-lg">
                      ...
                      of your salary remaining this month!
                    </p>
                    <p className="text-red-100 mt-2">
                      Consider reviewing your expenses and planning accordingly.
                    </p>
                    <p className="text-red-100 mt-1 text-sm">
                      📧 Email notification has been sent to your registered
                      email.
                    </p>
                  </div>
                </div>
